...
with aba2:
    ...
    if not animes_dados:
        st.info("Nenhum Anime disponível para atualização.", icon="ℹ️")
    else:
        opcoes = {f'{anim["id"]} - {anim["titulo"]}': anim for anim in animes_dados}
        escolha = st.selectbox("Selecione um anime para atualizar:", list(opcoes.keys()))

        anime_select = opcoes[escolha]

        with st.form("edita_animes"):
            novo_titulo = st.text_input("Titulo", value=anime_select["titulo"])
            nova_nota= st.number_input("Nota", value=anime_select['nota'])
            novo_ano_inicio = st.date_input("Ano de Lançamento", value=date.fromisoformat(anime_select["ano_inicial"]))
            novo_ano_final = st.date_input("Ano de Finalização", value=date.fromisoformat(anime_select["ano_final"]))
            atualizar_submitted = st.form_submit_button("Adicionar Nova Avaliação")
            if atualizar_submitted:
                datab.adicionar_avaliacao(any,nova_nota)
                datab.atualizar_anime(any,novo_titulo,novo_ano_inicio,novo_ano_final,nova_nota)
                st.success(f'Anime "{novo_titulo}" avaliado com sucesso!', icon="✅")
